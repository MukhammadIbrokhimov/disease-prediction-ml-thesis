...
Table~\ref{tab:training_times} shows the recorded wall-clock time (rounded) needed to fit the final model for each classifier on each dataset in the experiment runs. The reported values are the time to fit the final model once with the selected hyperparameters; they exclude the additional overhead of the full grid search with 5-fold stratified cross-validation used for tuning on Dataset~1 (Section~\ref{sec:hyperparameter_tuning}), which is substantially larger. On Dataset~2 no grid search was run, as exhaustive tuning was not computationally feasible at this scale. All the numbers vary with the system load.
