...
I made a confusion matrix for the XGBoost model on Dataset~1 to see how the model works with this data. I chose XGBoost over Random Forest because, when the two tree-based models are compared across the two datasets, XGBoost is the better of the two. The normalized confusion matrix is given in Figure~\ref{fig:confusion_matrix}, wherein cells along the diagonal are marked with the per-class accuracy.
