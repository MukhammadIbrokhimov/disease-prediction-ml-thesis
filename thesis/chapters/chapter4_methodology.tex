...
\node[data] (data) {%
    \textbf{Raw Data} \\
    Dataset~1: 4{,}920 records (304 unique), 131 symptoms, 41 diseases \quad\textbar\quad
    Dataset~2: $\sim$246{,}000 records ($\sim$189{,}600 used), 377 symptoms, 727 diseases};
...
Every patient is encoded as a binary vector of dimension $d = 131$ (for Dataset~1). A patient with fever, headache and nausea would have:
...
Dataset~2 is substantially imbalanced, while Dataset~1 is only mildly imbalanced (roughly 2:1) after deduplication. In both cases some diseases are more prevalent than others, and when this is not dealt with, classifiers tend to over-predict common diseases and ignore rare ones.
...
The two models, BernoulliNB and XGBoost, were not explicitly trained with \texttt{class\_weight} in scikit-learn, and the default unweighted loss was used for XGBoost. This is an important thing to note when you look at the accuracy column on Dataset~2 --- the models that had higher accuracy (Logistic Regression, SVM, Random Forest) sacrificed some accuracy for the rare classes, but Naive Bayes and XGBoost could opt for the more frequent diseases for better accuracy. The accuracy comparison is not exactly like for like, and the macro-averaged scores (which are averaged on an equal basis over the five models) provide a more balanced view of the accuracy.

\textbf{Stratified Sampling:} Class proportions are maintained in all train-test splits and cross-validation folds.

I had planned on using SMOTE~\citep{Chawla2002} but decided not to use it in this application. The original SMOTE creates synthetic examples by interpolating between neighbouring examples and then averages the binary symptom vectors to get fractional symptom presentations that do not exist. Later categorical/binary variants like SMOTE-N and SMOTE-NC were suggested for exactly this type of nominal data~\citep{Fernandez2018}, but these require further complexity and tuning. I liked class weighting, a simpler version of balancing that did not add any artificial data points.
