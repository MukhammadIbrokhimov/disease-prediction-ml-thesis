 \documentclass[12pt, a4paper, oneside]{report}

% ============================================================
% Page Layout (4cm binding margin on the left for hard binding, 2cm R, 2.5cm T, 3cm B)
% ============================================================
\usepackage[left=4cm, right=2cm, top=2.5cm, bottom=3cm]{geometry}

% ============================================================
% Font & Encoding
% ============================================================
\usepackage[T1]{fontenc}
\usepackage[utf8]{inputenc}
\usepackage{mathptmx}          % Times New Roman clone

% ============================================================
% Core Packages
% ============================================================
\usepackage{graphicx}           % Figures
\usepackage{booktabs}           % Professional tables
\usepackage{amsmath, amssymb}   % Math
\usepackage{hyperref}           % Clickable links
\usepackage{natbib}             % Harvard-style citations
\usepackage{setspace}           % Line spacing
\usepackage{parskip}            % Paragraph spacing
\usepackage{indentfirst}        % Indent first paragraph
\usepackage{float}              % Figure placement
\usepackage{caption}            % Caption formatting
\usepackage{enumitem}           % List customization
\usepackage{xcolor}             % Colors (for hyperref)
\usepackage{listings}           % Code listings
\usepackage{appendix}           % Appendix support
\usepackage{longtable}          % Multi-page tables
\usepackage{multirow}           % Table cell merging
\usepackage{pdflscape}          % Landscape pages
\usepackage{fancyhdr}           % Headers/footers
\usepackage{pdfpages}           % Embed official examination-office PDF forms
\usepackage{tikz}               % Diagrams (pipeline figure)
\usetikzlibrary{shapes.geometric, arrows.meta, positioning, calc}

% ============================================================
% University Branding Colors (UE Germany Official)
% ============================================================
\definecolor{UERed}{HTML}{F9423A}          % UE primary red
\definecolor{UEDoveBlue}{HTML}{3C4554}     % UE secondary dove blue
\definecolor{UEGrey}{HTML}{64666A}         % UE cool grey

% ============================================================
% Formatting
% ============================================================
\setlength{\parindent}{1cm}     % 1cm paragraph indent (per UE)
\onehalfspacing                 % 1.5 line spacing

\hypersetup{
    colorlinks=true,
    linkcolor=black,
    citecolor=black,
    urlcolor=blue,
    pdfauthor={Mukhammadsidik Ibrokhimov},
    pdftitle={Comparing Machine Learning Methods for Predicting Diseases from Symptoms in Healthcare}
}

% Header/footer
\pagestyle{fancy}
\fancyhf{}
\fancyhead[R]{\thepage}
\renewcommand{\headrulewidth}{0pt}

% ============================================================
% Document Metadata
% ============================================================
\title{Comparing Machine Learning Methods for Predicting Diseases from Symptoms in Healthcare}
\author{Mukhammadsidik Ibrokhimov}
\date{\today}

% ============================================================
\begin{document}

% --- Cover Page (unnumbered; titlepage env hides number) ---
\input{chapters/coverpage}

% --- Front Matter (Roman page numbers: i, ii, iii, ...) ---
\pagenumbering{roman}

% Statement of Authorship (official UE Examination Office form, filled)
\includepdf[pages=-, fitpaper=true, pagecommand={%
  \thispagestyle{empty}\phantomsection
  \addcontentsline{toc}{chapter}{Statement of Authorship}}]%
  {chapters/forms/authorship_filled.pdf}

% Declaration on AI Use (official UE Examination Office form, filled)
\includepdf[pages=-, fitpaper=true, pagecommand={%
  \thispagestyle{empty}\phantomsection
  \addcontentsline{toc}{chapter}{Declaration on the Use of AI}}]%
  {chapters/forms/ai_usage_filled.pdf}

\tableofcontents
\listoffigures
\listoftables

% List of Abbreviations
\input{chapters/abbreviations}

% Abstract
\input{chapters/abstract}

% --- Main Content (Arabic page numbers, starting at 1) ---
\clearpage
\pagenumbering{arabic}
\setcounter{page}{1}

\input{chapters/chapter1_introduction}
\input{chapters/chapter2_literature_review}
\input{chapters/chapter3_data_description}
\input{chapters/chapter4_methodology}
\input{chapters/chapter5_results}
\input{chapters/chapter6_discussion}
\input{chapters/chapter7_conclusion}

% --- Acknowledgements ---
\input{chapters/acknowledgements}

% --- Bibliography ---
\bibliographystyle{apalike}
\bibliography{references}

% --- Appendix ---
\begin{appendices}
\input{chapters/appendix_a}
\end{appendices}

\end{document}
